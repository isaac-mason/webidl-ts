  ...
  idl.members.forEach((member: webidl2.IDLInterfaceMemberType | webidl2.FieldType) => {
    switch (member.type) {
      case 'attribute':
        ...
        if (options.emscripten && member.type === 'attribute' && member.idlType.generic === 'FrozenArray') {
          // array attributes in emscripten bind the property value to the the first item in the array
          members.push(
...
function createEmscriptenAttributeGetter(value: webidl2.AttributeMemberType) {
  const isArrayAttribute = value.type === 'attribute' && value.idlType.generic === 'FrozenArray'

  const idlType = isArrayAttribute ? (value.idlType.idlType[0] as unknown as webidl2.IDLTypeDescription) : value.idlType

  const type = convertType(idlType)

  const parameters: ts.ParameterDeclaration[] = isArrayAttribute
    ? [
        ts.factory.createParameterDeclaration(
          [],
          undefined,
